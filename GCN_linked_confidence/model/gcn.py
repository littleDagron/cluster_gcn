...
class gcn(nn.Module):
    def __init__(self):
        ...
        self.dim = 100
        self.halfdim = int(self.dim/2)
        self.bn0 = nn.BatchNorm1d(self.dim, affine=False)
        self.conv1 = GraphConv(self.dim, self.dim, MeanAggregator)
        self.conv2 = GraphConv(self.dim, self.dim, MeanAggregator)
        self.conv3 = GraphConv(self.dim, self.halfdim, MeanAggregator)
        self.conv4 = GraphConv(self.halfdim, self.halfdim, MeanAggregator)

        self.classifier = nn.Sequential(
            nn.Linear(self.halfdim, self.halfdim),
            nn.PReLU(self.halfdim),
            nn.Linear(self.halfdim, 2))
